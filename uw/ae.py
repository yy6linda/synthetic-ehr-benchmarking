...
class AE(tf.keras.Model):
    def __init__(self,size):
        ...
        self.decoder = tf.keras.layers.Dense(size, activation=tf.nn.sigmoid)
...
def main(modeln):
    data = np.load('data/train_'+modeln+'.npy').astype('float32')
    np.random.shuffle(data)
    size = data.shape[-1]
    data = tf.data.Dataset.from_tensor_slices(data).shuffle(10000, reshuffle_each_iteration=True).batch(1000,drop_remainder=True)
    model = AE(size)
    optimizer = AdamWeightDecay(1e-3, weight_decay_rate=0.001)
...
if __name__ == "__main__":
    ...
    for modeln in ['pos','neg','combined']:
        checkpoint_directory = "training_checkpoints_ae_" + modeln
